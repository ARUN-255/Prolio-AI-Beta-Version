...
function ATSChecker() {
  ...
  const handleAnalyze = async (event) => {
    ...
    try {
      setAnalyzing(true);
      setError("");
      const data = await runAtsAnalysis({
    ...
    <div className="ats-page">
      <section className="ats-page-heading">
        <div>
          ...
          <p>Choose one of your resumes, paste the job description and see what matches and what needs improvement.</p>
        </div>
      </section>
      ...
      <div className="ats-layout">
        <form className="ats-input-card" onSubmit={handleAnalyze}>
          <div className="ats-card-heading">
            ...
            <div><h2>New analysis</h2><p>Compare one saved resume with a job description.</p></div>
          </div>

          {resumes.length === 0 ? (
            <div className="ats-no-resume">Create a resume first before running an ATS check.</div>
          ) : (
            <>
        ...
        <section className="ats-result-card">
          {!result ? (
            <div className="ats-result-empty"><FileCheck2 size={34} /><h2>Your result will appear here</h2><p>Run an analysis to see the ATS score, keyword gaps, strengths and improvements.</p></div>
          ) : (
            <>
              <div className="ats-score-row">
                <div className="ats-score-circle"><strong>{result.ats_score ?? 0}</strong><span>/100</span></div>
                <div><p className="eyebrow">ATS match</p><h2>{result.job_title || "Job analysis"}</h2>{quota && <p className="ats-quota">Checks used: {quota.used}{quota.unlimited ? " · Unlimited" : ` / ${quota.limit}`}</p>}</div>
              </div>
              ...
              {result.ai_feedback && <article className="ats-ai-card"><div className="ats-card-heading"><span className="ats-icon"><Sparkles size={19} /></span><div><h3>AI feedback</h3><p>Additional guidance based on the job description.</p></div></div><pre>{typeof result.ai_feedback === "string" ? result.ai_feedback : JSON.stringify(result.ai_feedback, null, 2)}</pre></article>}
            </>
          )}
        </section>
      </div>
